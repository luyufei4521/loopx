...
def main() -> int:
    with tempfile.TemporaryDirectory(prefix="loopx-project-agent-adoption-") as tmp:
        ...
        # The planner offers the current priority band, so the machine's own
        # authoring template has to teach the marker that makes a row bindable.
        assert "[P1]" in approved_quota["todo_write_hint"]["agent_todo_command_template"], approved_quota
        assert "[P1]" in approved_quota["todo_write_hint"]["agent_todo_command_template"], approved_quota
